...
with tab1:
    ...
    st.caption("""Lawrence's Hispanic population is predominantly Dominican, with 46,138 residents identifying as Dominican—nearly three times the size of the next-largest group, Puerto Ricans (15,769). Central American (5,677) and Guatemalan (3,711) populations represent smaller but notable communities, while Salvadoran, South American, and Mexican populations each account for a relatively small share of residents. Together, these figures illustrate Lawrence's strong Dominican cultural presence while highlighting the diversity of Hispanic backgrounds represented throughout the city.""")
    col1, col2 = st.columns([2,1])
...
with tab2:
    ...
    st.caption("Most residents have either a regular high school diploma or some level of college experience, making these the most common educational outcomes in the community. A substantial number of individuals have not completed high school, while bachelor's degree holders represent a significant portion of the population. In contrast, advanced degrees such as master's, professional, and doctoral degrees are relatively rare. Overall, the population is primarily composed of individuals with high school-level education, some college education, or bachelor's degrees, with fewer residents holding graduate-level credentials.")
    col1, col2 = st.columns([2, 1])
...
with tab3:
    ...
    female_workers = cleaned_employ_gender_earnings.loc[
        cleaned_employ_gender_earnings["groups"] == "Female full-time year-round workers",
        "values"
    ].iloc[0]
    ...
    hs_salary = cleaned_employ_ed_earnings.loc[
        cleaned_employ_ed_earnings['groups'] == 'High school graduate (includes equivalency)',
        'values'
    ].iloc[0]
    ...
    bachelor_salary = cleaned_employ_ed_earnings.loc[
        cleaned_employ_ed_earnings['groups'] == "Bachelor's degree",
        'values'
    ].iloc[0]
    ...
    st.caption("Most workers in Lawrence have relatively short commutes, with the largest groups traveling 10–14 minutes (8,144 people) and 15–19 minutes (7,050 people) to work. Commutes of 20–24 minutes (5,563 people) and 30–34 minutes (4,158 people) are also common. While some residents travel 45 minutes or more, long commutes are less frequent overall, indicating that many workers live fairly close to their workplaces.")
    col1, col2 = st.columns([2, 1])

    with col1:
        st.plotly_chart(plot_com_length(cleaned_employ_com_length), use_container_width=True)
